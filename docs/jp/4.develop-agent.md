...
### 4.7 ローカル環境での Web アプリケーションの実行
```bash
python app.py
```

### 4.8 Azure Web Apps へのデプロイ
```bash
RESOURCE_GROUP="" # デプロイ先のリソースグループ名
APP_PLAN_NAME=""  # 作成する App Service Plan の名前
APP_PLAN_SKU="B1" # 作成する App Service Plan のプラン
APP_PLAN_REGION="japaneast"   # デプロイ先のリージョン
WEB_APP_NAME=""               # 作成する Web Apps の名前
WEB_APP_RUNTIME="PYTHON:3.11" # 作成する Web Apps のランタイム

az webapp up \
    --resource-group $RESOURCE_GROUP \
    --location $APP_PLAN_REGION \
    --plan $APP_PLAN_NAME \
    --sku $APP_PLAN_SKU \
    --name $WEB_APP_NAME \
