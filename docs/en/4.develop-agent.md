...
Then, paste the AI Project connection string you copied into the value of ```AZURE_AI_AGENT_PROJECT_CONNECTION_STRING``` in the .env file as shown below:
...
Verify that the AI Search account you selected has been added. Copy this name (the resource connection name) (in the example below it is "ragagenthandson250401") and paste it into the ```AI_SEARCH_CONNECTION_NAME``` value in the ```src/.env``` file. Also, adjust the value of ```AZURE_AI_AGENT_MODEL_DEPLOYMENT_NAME``` according to the name of the GPT model you will use.
